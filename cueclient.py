...
class Client:
    ...
    def post(self, flag_names):
        for name in flag_names:
            url = self.config.server_url / "cues" / name
            headers = {"AUTHORIZATION": f"Bearer {self.config.token}"}
            r = requests.post(url, json={"name": "yo"}, headers=headers)
            r.raise_for_status()
    ...
    async def async_iter(self, flag_name):
        while True:
            socket_url = (self.config.socket_url / "listen").with_query({"name": flag_name})
            headers = {"AUTHORIZATION": f"Bearer {self.config.token}"}
            log.debug(f"Connecting: {socket_url}")
            try:
                async with websockets.connect(str(socket_url), extra_headers=headers) as ws:
